...
#from H36M_VAEAnalogyModel import VAEAnalogyModel
...
def run_visualization(video_proc_lib, video_dir, video_idx, file_suffix,
  ...
  # 
  #AB_imgs_left = np.concatenate((A_imgs, np.tile(A_imgs[-1:], (fut_len, 1, 1, 1))), axis=0)
  ...
  #
  #CD_imgs_left = np.concatenate((C_imgs, np.tile(C_imgs[-1:], (fut_len, 1, 1, 1))), axis=0)
...
def main(_):
  ...
  with g.as_default():
    ...
    with tf.Session() as sess:
      ...
      #  (dataset_size, params.max_length, params.keypoint_dim, 2), dtype=np.float32)
      #
