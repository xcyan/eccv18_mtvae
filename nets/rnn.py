...
Code adapted from Magenta Project."""
...
class HyperLSTMCell(tf.contrib.rnn.RNNCell):
  ...
  def get_output(self, state):
    unused_total_c, total_h = tf.split(state, 2, 1)
  ...
  def __call__(self, x, state, timestep=0, scope=None):
    with tf.variable_scope(scope or type(self).__name__):
      ...
      hyper_c, hyper_h = tf.split(hyper_new_state, 2, 1)
      new_total_h = tf.concat([new_h, hyper_h], 1)
      new_total_c = tf.concat([new_c, hyper_c], 1)
      new_total_state = tf.concat([new_total_c, new_total_h], 1)
